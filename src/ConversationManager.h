...
class ConversationManager
{
    ...
    bool canSpeak(const RoomSpeakConfig& config, bool baseSpeakEnabled) const;
    ...
    bool isPhaseRequirementMet(const ConversationPhase& phase) const;
    const ConversationPhase* findPhase(const RoomSpeakConfig& config, const std::string& phaseId) const;
    const ConversationPhase* findActivePhase(const std::string& roomId, const RoomSpeakConfig& config) const;
