...
void TakeMgr::setUiBackdrop(const UiBackdrop* backdrop)
{
    uiBackdrop = backdrop;
}
...
void TakeMgr::drawItemGrid() const
{
    ...
    for (size_t i = 0; i < slots.size() && i < itemSlotBounds.size(); ++i)
    {
        ...
        DrawRectangleRounded(slot, 0.18f, 8, hovered ? slotHover : slotFill);
...
void TakeMgr::draw() const
{
    ...
    const float pad = 14.0f;
    DrawTextEx(panelFont, "TAKE", { panelBounds.x + pad, panelBounds.y + pad }, 17.0f, 1, sectionLabel);
